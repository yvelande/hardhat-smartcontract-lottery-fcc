...
// import { assert, expect } from "../../node_modules/chai/chai"
    ...
    : describe("Raffle Unit Tests", function () {
          ...
          describe("checkUpkeep", function () {
              it("returns false if people haven't sent any ETH", async () => {
                  await network.provider.send("evm_increaseTime", [Number(interval) + 1])
                  await network.provider.request({ method: "evm_mine", params: [] })
                  const { upkeepNeeded } = await raffle.checkUpkeep.staticCall(new Uint8Array()) // upkeepNeeded = (timePassed && isOpen && hasBalance && hasPlayers)
                  assert(!upkeepNeeded)
              })
              //   })
              // })
                  ...
                  await new Promise(async (resolve, reject) => {
                      raffle.once("WinnerPicked", async () => {
                          ...
                          try {
                              ...
                              const winnerBalance = await await ethers.provider.getBalance(
                                  accounts[2],
                              )
